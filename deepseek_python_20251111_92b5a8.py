...
async def start_handler(client: Client, message: Message):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📖 Help", callback_data="help"),
         InlineKeyboardButton("☁️ Storage Info", callback_data="storage_info")],
        [InlineKeyboardButton("🔧 Supported Sites", callback_data="supported_sites")]
    ])
    
    await message.reply_text(
        "🚀 **Universal Download Bot**\n\n"
        "I can download from:\n"
        "• 🌐 Direct URLs\n"
        "• 📺 YouTube/1000+ sites\n"
        ...
        "• /torrent <magnet/torrent> - Download torrent\n"
        "• /ytdl <url> - YouTube-DL download\n"
...
async def download_handler(client: Client, message: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("📱 Telegram", callback_data=f"upload_telegram_{filename}"),
             InlineKeyboardButton("☁️ Wasabi", callback_data=f"upload_wasabi_{filename}")],
...
async def ytdl_handler(client: Client, message: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🎥 Video", callback_data=f"ytdl_{quality}_{url}"),
             InlineKeyboardButton("🎵 Audio Only", callback_data=f"ytdl_audio_{url}")],
            [InlineKeyboardButton("❌ Cancel", callback_data="cancel")]
        ])
        
        await status_msg.edit_text(
            f"🎬 **{info.get('title', 'Unknown')}**\n"
            f"⏱ Duration: {info.get('duration', 'Unknown')}s\n"
            f"📊 Quality: {quality}\n\n"
        ...
        await status_msg.edit_text(f"❌ Failed to get video info: {str(e)}")
...
async def torrent_handler(client: Client, message: Message):
    ...
    if not (is_magnet_link(torrent_input) or is_torrent_file(torrent_input)):
        return await message.reply_text("❌ Please provide a valid magnet link or .torrent URL")
    
    status_msg = await message.reply_text("🧲 Initializing torrent download...")
    
    try:
        download_path = f"torrents/{int(time.time())}"
        os.makedirs(download_path, exist_ok=True)
        
        await status_msg.edit_text("📥 Downloading torrent...")
        ...
        await status_msg.edit_text(f"❌ Torrent download failed: {str(e)}")
...
async def callback_handler(client: Client, callback_query):
    ...
    if data == "help":
        ...
        await message.edit_text(
            ...
            "• `/ytdl <url> [quality]` - YouTube/1000+ sites\n"
            "• `/torrent <magnet/torrent>` - Torrent download\n"
            "• `/upload` - Upload replied file\n\n"
            "**Quality options for ytdl:**\n"
            "• `best` - Best quality\n"
            "• `720p`, `480p` - Specific resolution\n"
            "• `audio` - Audio only\n\n"
...
async def handle_ytdl_download(client, message, url, quality):
    ...
    try:
        ...
        if files:
            ...
            keyboard = InlineKeyboardMarkup([
                [InlineKeyboardButton("📱 Telegram", callback_data=f"upload_telegram_{downloaded_file.name}"),
                 InlineKeyboardButton("☁️ Wasabi", callback_data=f"upload_wasabi_{downloaded_file.name}")]
            ])
            ...
            await message.edit_text("❌ No files downloaded")
...
async def handle_upload_callback(client, callback_query, data):
    ...
    try:
        ...
        if service == "telegram":
            ...
            await callback_query.message.edit_text("✅ Uploaded to Telegram successfully!")
            
        elif service == "wasabi" and SUPPORTED_CLOUDS['wasabi']['enabled']:
            await callback_query.message.edit_text("☁️ Uploading to Wasabi...")
